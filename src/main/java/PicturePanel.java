...
public class PicturePanel extends JPanel {
    ...
    public void brightnessChange(float percentage) throws IOException {
        ...
        for (int i = 0; i < image.getWidth(); i++) {
            for (int j = 0; j < image.getHeight(); j++) {
                ...
                g = (color & 0xff00) >> 8;
                r = (color & 0xff0000) >> 16;
                c = this.validateColor((int) (r * percentage), (int) (g * percentage), (int) (b * percentage));
                image.setRGB(i, j, c.getRGB());
            }
        }
        Graphics graphics = getGraphics();
    ...
    public void grayScale() throws IOException {
        for (int i = 0; i < image.getWidth(); i++) {
            for (int j = 0; j < image.getHeight(); j++) {
                ...
                r = (color & 0xff0000) >> 16;
                int avg = (r + g + b) / 3;
                c = this.validateColor(avg, avg, avg);
                image.setRGB(i, j, c.getRGB());
            }
        }
        Graphics graphics = getGraphics();
    ...
    public void grayScaleByRed() throws IOException {
        for (int i = 0; i < image.getWidth(); i++) {
            for (int j = 0; j < image.getHeight(); j++) {
                int color = image.getRGB(i, j);
                Color c = new Color(color);
    ...
    public void customFilter(){
        ...
        for (int i = 0; i < image.getWidth(); i++) {
            for (int j = 0; j < image.getHeight(); j++) {
                filteredPixels[i][j] = customize(i, j);
    ...
    public int[] customize(int x, int y){
        int elementsCount = 0;
        int[] average = new int[3];
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                try {
                    if (i == -1 && j == -1) {
                        ...
                        average[0] += pixels[x + i][y + j][0]*3;
                        average[1] += pixels[x + i][y + j][1]*3;
                        average[2] += pixels[x + i][y + j][2]*3;
                        elementsCount+=3;
                    } else if (i == 1 && j == 1) {
                        average[0] += pixels[x + i][y + j][0]*7;
                        average[1] += pixels[x + i][y + j][1]*7;
                        average[2] += pixels[x + i][y + j][2]*7;
                        elementsCount+=7;
                    }
